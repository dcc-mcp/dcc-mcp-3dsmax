...
def main(
    ...
    try:
        ...
        if count is not None and count != len(sections):
            if created_node:
                delete_node(rt, loft)
            return curve_error(
                "the loft registered {} of {} cross-sections".format(count, len(sections)),
                requested_cross_sections=len(sections),
        ...
        if path_node:
            ...
            if not ok:
                if created_node:
                    delete_node(rt, loft)
                return curve_error(error, rolled_back=created_node)
