...
def main(
    ...
    """Apply an Unwrap UVW modifier to explicit targets."""
    ...
    for node in targets["objects"]:
        warnings.extend(apply_unwrap(rt, node, channel=int(channel), operation="unwrap", padding=None))
    return uv_success("Applied Unwrap UVW", nodes=changed_channels(rt, targets["objects"]), warnings=warnings)
