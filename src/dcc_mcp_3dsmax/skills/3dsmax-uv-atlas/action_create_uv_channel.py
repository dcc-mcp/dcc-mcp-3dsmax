...
from dcc_mcp_3dsmax._uv_atlas import changed_channels, create_channel, resolve_uv_targets, uv_success, validate_channel
...
def main(
    ...
    for node in targets["objects"]:
        warnings.extend(create_channel(rt, node, int(channel)))
    return uv_success("Created UV channel", nodes=changed_channels(rt, targets["objects"]), warnings=warnings)
