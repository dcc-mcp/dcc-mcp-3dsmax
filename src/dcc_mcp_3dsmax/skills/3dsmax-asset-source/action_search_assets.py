...
def search_assets(query: str, limit: int = 10) -> Dict[str, Any]:
    ...
    Returns:
        ActionResult dict with matched descriptors in ``data.results``.
    """
    query = query.strip()
    if not query:
        return {
    ...
    return {
        "success": True,
        "status": "success",
        "message": f"Found {len(results)} asset(s) for '{query}'",
        "data": {
            "results": descriptors,
            "scores": scores,
            "total": len(results),
...
def main(query: str, limit: Optional[int] = None, **kwargs: Any) -> Dict[str, Any]:
    """Entry point for 3ds Max MCP tool dispatch."""
    return search_assets(query=query, limit=limit or 10)
