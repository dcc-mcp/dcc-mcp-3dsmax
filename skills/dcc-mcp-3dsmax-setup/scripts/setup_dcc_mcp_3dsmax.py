...
def build_startup_script(*, source: str, repo_root: Path) -> str:
    ...
    log_dir = str((repo_root / ".dcc-mcp" / "bootstrap-errors").resolve())
    encoded_log_dir = base64.b64encode(log_dir.encode("utf-8")).decode("ascii")
    python_code = (
        ...
        + "        matching = [name for name in names if re.sub('[-_.]+', '-', name).lower() == expected]\n"
        + "        if not matching:\n"
        + "            continue\n"
        + "        if len(names) != 1 or len(matching) != 1 or len(versions) != 1:\n"
        ...
        + "    core = _release_version(_distribution_version(dcc_mcp_core, 'dcc-mcp-core'))\n"
        + "    server = _release_version(_distribution_version(dcc_mcp_server, 'dcc-mcp-server'))\n"
